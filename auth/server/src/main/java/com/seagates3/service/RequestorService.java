        ...
        try {
            ...
            if (dataObj != null) {
              long differnce =
                  ...
                  1000;
              LOGGER.info("time difference in seconds is - " + differnce);
              if (differnce < 30) {  // if entry in cache is older than 30
                                     // seconds then refresh it
                return dataObj.getRequestor();
              }
            }
            LOGGER.info("fetching from LDAP as 30 seconds over");
